...
Install it from the marketplace — two lines in Claude Code:

```text
/plugin marketplace add latent-spaces/nanoclip-cutting-room
/plugin install nanoclip-cutting-room@nanoclip
```

Then point it at footage:

```text
cut ~/footage/ep7.mp4 into shorts
```

That is the whole setup. Claude checks the machine (node 20+, ffmpeg — installs
the [NanoClip CLI](https://www.npmjs.com/package/nanoclip) from npm when it is
missing), tells you if `nanoclip auth login` is needed, quotes the spend in
cents, and waits for your approval before anything runs.

Prefer your own hands, or want the demo below without a plugin install:

```bash
git clone https://github.com/latent-spaces/nanoclip-cutting-room.git ~/.claude/skills/nanoclip-cutting-room
```

`~/.claude/skills/` auto-loads on the next session, subagents included — same
thing the marketplace gives you, one directory you own.
...
Run it from the clone above (the marketplace install keeps the same files under
`~/.claude/plugins/`, if you'd rather point node there).
...
agents/         Composer / Finisher — registered subagent types (composer, finisher)
