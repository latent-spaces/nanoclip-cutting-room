...
export const CUE_YIELD_S = 0.05;  // clearance before the next cue takes over
...
export function deriveCues(clip, words) {
  ...
  for (const seg of clip.segments) {
    ...
    for (const w of words) {
      ...
      const end = round2(base + (Math.min(w.end, seg.src_out) - seg.src_in));
      const fresh = !cue
  ...
  cues.forEach((cue, i) => {
    ...
    cue.end = round2(Math.min(
      cue.words.at(-1).end + CUE_HOLD_S,
      next ? next.start - CUE_YIELD_S : Infinity,
      total,
    ));
